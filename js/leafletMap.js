class LeafletMap {
  ...
  initVis() {
	...
	vis.ShapePane = vis.theMap.createPane("shapePane");
	vis.ShapePane.stylezIndex = 450;

	//vis.polylineLayer = L.polyline([], { pane: "shapePane" }).addTo(vis.theMap);
	vis.polylines = [];
  ...
  updateVis(newColorScale = null) {
   ...
   //redraw based on new zoom- need to recalculate on-screen position
                    ...
                    .join('circle')
                        ...
                        .on('click', (event, d) => {
                        	...
                        	if (d.ROOT > 0) {
                        		...
                        		vis.updatePolygon(d, false); // Move out of if to show ones that don't have additional assets
    ...
    //vis.addPolylines(null, [], false);  /// NO CLUE WHAT THIS IS FOR //////
  ...
  addPolylines(asset, children, reset) {
  	let vis = this;
  	...
  	if (reset == true) {
  		...
  		//vis.polylines.splice(0, vis.polylines.length);

  		vis.polylines.forEach(function(d) {
  			vis.theMap.removeLayer(d);
  		});

  		vis.polylines.splice(0, vis.polylines.length);
  		//vis.theMap.removeLayer(vis.polylineLayer);
  	...
  	//vis.polylineLayer = L.polyline(vis.polylines, { color: "blue", pane: "shapePane" }).addTo(vis.theMap);
  	
  	//vis.polylineLayer.setLatLngs(vis.polylines);
  	
  	//vis.polylineLayer.setStyle({color: "white"});
  	
  	// vis.polylineLayer.setStyle({
    //                 color: "white",
    //                 weight: 4
    //             });
